...
class AbbrManager(object):
	...
	def __init__(self, db_file = "", logger = None):
		super(AbbrManager, self).__init__()
...
class AbbrHelper(object):
	"""docstring for AbbrHelper"""
	
	def __init__(self, db_file = None, logger = None):
		super(AbbrHelper, self).__init__()
...
class AbbrHelperWebApp(object):
	...
	def __init__(self):
		...
		self.LOG_FILE = "/var/log/abbr_helper.log"
	...
	def run_web_interface(self):
		...
		def show_db():
			if request.method == "GET":
				return render_template("show_db.html", db = self.main_app.abbr_manager.abbr_db_as_list)
		...
		def edit_abbr(abbr_id):
			...
			if request.method == "POST":
				...
				# abbr = {}
				try:
					abbr["abbr"] = request.form["abbreviation"]
					...
					self.main_app.abbr_manager.save_abbr(abbr)
				except Exception as e:
		...
		def delete_abbr(abbr_id):
			...
			if request.method == "GET":
				return render_template("delete_abbr.html", abbr = found_abbr["abbr"] + " - " + found_abbr["descr"])
			if request.method == "POST":
				self.main_app.abbr_manager.delete_abbr_by_id(abbr_id)
				return render_template("main_page.html")
